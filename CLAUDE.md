...
- **`store`** (`project-store.ts`) — the ACTIVE project's `ProjectData` model (`model.ts`), file IO, and audio-buffer cache. Debounced autosave writes `project.json` into the active project's directory and mirrors it to IndexedDB. The **project manager** (`project-manager.ts`, singleton `projects`) sits above it: owns the user-picked ROOT folder (one subdirectory per project, each portable: `project.json` + `ui.json` + `keymap.json` + WAVs), handles boot restore, project switching/creation (dropdown in the shell), and `saveAll()` (Save button / Ctrl+S full flush). The app works fully without a root folder via per-project IndexedDB mirrors (`project:<name>:data` / `:ui` keys). Also owns the path→`AudioBuffer` cache; WAVs live in `tones/`, `samples/`, `sequences/`, `exports/` inside the project folder. Two mutation styles: `store.update(fn)` notifies listeners via `project:changed` + saves; bare mutation + `store.scheduleSave()` saves without re-render (used by knobs to avoid destroying the widget mid-drag).
...
**UI state** (`src/core/ui-state.ts`): all transient UI state (active tab, selections, toggles) is part of the project — persisted via `updateUi(fn)` through a persister the project manager registers (IndexedDB mirror + the project's `ui.json`). The manager emits `ui:loaded` then `project:loaded` at boot AND on every project switch; each module re-applies its slice in a `bus.on('ui:loaded', …)` handler. When adding new UI state (a selection, a toggle), route it through `UiState`, don't keep it only in a class field.
